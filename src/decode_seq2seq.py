...
import pickle
import sys
sys.path.insert(0, r"E:\py-workspace\summary\unilm\src")
...
from biunilm import decode_with_rule
...
def main():
    ...
    pair_num_relation = 0
    bi_uni_pipeline = []
    bi_uni_pipeline.append(seq2seq_loader.Preprocess4Seq2seqDecoder(list(tokenizer.vocab.keys()), tokenizer.convert_tokens_to_ids,
                                                                    args.max_seq_length, max_tgt_length=args.max_tgt_length,
                                                                    new_segment_ids=args.new_segment_ids, mode="s2s",
                                                                    num_qkv=args.num_qkv, s2s_special_token=args.s2s_special_token,
                                                                    s2s_add_segment=args.s2s_add_segment, s2s_share_segment=args.s2s_share_segment,
                                                                    pos_shift=args.pos_shift,from_source=args.from_source))
    ...
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        ...
        model = BertForSeq2SeqDecoder.from_pretrained(args.bert_model, state_dict=model_recover, num_labels=cls_num_labels,
                                                      ...
                                                      use_rule=args.use_rule, rule=args.rule, local_debug=args.local_debug)
        del model_recover
        ...
        with open(args.input_file, encoding="utf-8") as fin:
            raw_lines = [x.strip() for x in fin.readlines()]
            if args.subset > 0:
                logger.info("Decoding subset: %d", args.subset)
                input_lines = input_lines[:args.subset]
        data_tokenizer = WhitespaceTokenizer() if args.tokenized_input else tokenizer
        input_lines = [data_tokenizer.tokenize(x) for x in input_lines]

        logger.info("数据样本个数: %d", len(input_lines))
        tmp = []
        index_list = []
        for idx,line in enumerate(input_lines):
            ...
            for w in line:
                ...
                if w in BIG_SPLIT_TOKENS:
                    if len(s) + len(a) < max_src_length and len(s) > 1:
                        a.extend(s)
                    s = []
            if len(a) < max_src_length and len(a) > 0 and a[-2] not in TOTAL_SPLIT_TOKENS:
                tmp.append(a)
        ...
        with tqdm(total=total_batch) as pbar:
            while next_i < len(input_lines):
                ...
                with torch.no_grad():
                    ...
                    input_ids, token_type_ids, position_ids, input_mask, mask_qkv, task_idx, decode_mask = batch
                    if args.use_rule and args.rule in [0,1,2]:
                        traces = decode_with_rule.run_beam_search(model, tokenizer, input_ids, token_type_ids,
                                   position_ids, input_mask,decode_mask,task_idx=task_idx, mask_qkv=mask_qkv, args=args, rule=[0,1,2])

                    else:
                        traces = model(input_ids, token_type_ids,
                                   position_ids, input_mask,decode_mask,task_idx=task_idx, mask_qkv=mask_qkv)
                    # unilm beam search
                    if args.rule == 3:
        ...
        decode_input = []
        with open(args.input_file, encoding='utf-8') as fin:
            decode_input = [x.strip() for x in fin.readlines()]
            if args.subset > 0:
                decode_input = decode_input[0:args.subset]
        tmp = []
        for i in index_list:
            tmp.append(decode_input[i])
        decode_input = tmp
        assert len(decode_input) == len(output_lines)
        args.output_file = args.output_file[0:args.output_file.index('.')] + '_{}_{}.txt'.format(args.subset, args.search_beam_size)
        with open(args.outpt_file, 'w', encoding='utf-8') as f:
            for i in range(len(decode_input)):
                line = 'Article:\n' + decode_input[i] +'\nSummary:' + output_lines[i] +'\n'
                f.write(line)
            logger.info('完成')
        # if args.output_file:
